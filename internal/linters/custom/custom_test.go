...
func TestMyAnalyzer(t *testing.T) {
	analysistest.Run(t, analysistest.TestData(), NoMainExit, "./...")
}

func Example_chechListLinterStaticcheck() {
	defaultAnalysis := []*analysis.Analyzer{
		asmdecl.Analyzer,
		assign.Analyzer,
		atomic.Analyzer,
	...
	for _, v := range defaultAnalysis {
		shortDoc, _ := helpers.SplitPrefixLine(v.Doc)
		fmt.Printf("%s [%s]\n", v.Name, shortDoc)
	}
	sort.Slice(defaultAnalysis, func(i, j int) bool {
		return defaultAnalysis[i].Name < defaultAnalysis[j].Name
	})

	all := helpers.Concatenate(sa.Analyzers, s.Analyzers, st.Analyzers, qf.Analyzers)
	sort.Slice(all, func(i, j int) bool {
		return all[i].Analyzer.Name < all[j].Analyzer.Name
	})
	for _, v := range all {
		shortDoc, _ := helpers.SplitPrefixLine(v.Analyzer.Doc)
		fmt.Printf("%s [%s]\n", v.Analyzer.Name, shortDoc)
	}
